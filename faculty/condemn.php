...
// ✅ Fetch logged-in faculty full name
$userQuery = mysqli_query($conn, "SELECT CONCAT(firstname, ' ', lastname) AS fullname FROM account WHERE employee_id='$user_id'");
$userData = mysqli_fetch_assoc($userQuery);
$faculty_name = $userData['fullname'] ?? '';

// ✅ Fetch condemned history (only accepted requests by this faculty)
$query = "
SELECT 
    cr.requested_by,
    cr.date_requested,
...
WHERE cr.requested_by = '$faculty_name'
  AND cr.request_condemn = 0
...
<div class="container mt-4">
    <h2 class="mb-3"><i class="bi bi-archive"></i> Condemned Equipment History</h2>
    ...
    <?php if (mysqli_num_rows($groups) > 0): ?>
        ...
        <?php endwhile; ?>
    <?php else: ?>
        <div class="alert alert-info">No condemned equipment history found.</div>
    <?php endif; ?>
</div>
